...
import qualified Data.IntMap               as M (elems, filter, keys, lookup,
                                                 singleton)
import           Data.IORef                (IORef, newIORef)
import           Data.Set                  (Set)
import qualified Data.Set                  as S (empty, filter, insert, toList)
import           Data.Time.Clock           (UTCTime, getCurrentTime)
import           GHC.Generics              (Generic)

import           Entity.Book               (Book (Book, createdAt, title),
                                            BookRepository (findAll))
import           Entity.Lending            (Lending (Lending), LendingRepository (delete, findByBook, findByUser, save))
import qualified Entity.Lending            as L (book, user)
